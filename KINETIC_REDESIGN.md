...
- Three.js uses one persistent Canvas and 16 original Blender assets (689,796 bytes combined). Hidden scenes pause. Camera, node transforms, and spline geometry interpolate with section/selection state. The hero core follows the cursor, a Blender scroll runner passes vertically with scroll direction, and identity/project cards have 3D hover treatment. Mobile uses reduced geometry and simple signal materials; reduced-motion, low-end hardware, failed assets, and lost WebGL use the FZ SVG. Project modules unfold on hover and navigate to the selected real project. Editable `.blend` files and the generator are documented in `assets/blender/README.md`.
...
- `node scripts/qa/check-models.mjs`: all 16 GLB files passed structure, vertex, material, uniqueness, animation-target, and size checks.
