...
- Three.js renders original Blender assets through one persistent Canvas shared by the scroll states. Desktop, tablet, and mobile have separate quality levels; reduced motion, low-end hardware, failed models, or lost WebGL use authored SVG. DOM capability selection drives the models and camera; the hero core follows the cursor, a scroll-runner craft crosses each scene with scroll direction, identity cards have 3D hover treatment, and each published tech stack skill can show a matching Blender badge. Project explorer centers the real website preview image. See [Blender asset pipeline](assets/blender/README.md) for editable sources and regeneration commands.
...
The Project section now uses the [FZH Kinetic Device Showcase](docs/FZH_Kinetic_Device_Showcase_README.md): distinct monitor/laptop/mobile chassis with measured rendering and texture improvements.
